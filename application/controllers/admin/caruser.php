...
class Caruser extends BaseController{
	...
	public function index()
	{
		...
		$this->view('caruser/index.php', $viewData);
	}
	...
	public function award()
	{
		...
		$saveData['is_prize'] = 1;
		$userInfo = $this->Car_game_user_model->getByUserId($userId);
		if(!empty($userInfo)){
			if(empty($userInfo['lottery_name'] !=='')){
				$this->Car_game_user_model->updateById($saveData, $userId);
	...
	public function batch_delete()
    {
        ...
        foreach ($ids_arr as $value) {
            $result = $this->Car_game_user_model->deleteById($value);
            if (!$result) {
