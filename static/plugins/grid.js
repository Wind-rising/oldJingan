    ...
    return {
        initGridDefault: function () {
            ...
            $.jgrid.extend({
                bindKeys: function (b) {
                ...
                }, exportExcelLocal: function (b,L) {
                    this.each(function () {
                        var f = this;
                        if (!f.grid) {
                            return
                        }
                        var records = $(f).jqGrid('getGridParam', 'records');
                        var selarrow = $(f).jqGrid('getGridParam', 'selarrrow');
                        if(selarrow.length<1){
                            if(records>10000){
                                if(!confirm('当前记录大于一万条，确认导出?')){
                                    return
                                }
                            }
                        }
                        $(f).jqGrid('excelExport', {
                            tag: 'excel',
                            exptype: "remote",
                            url: Util.AddOrReplaceUrlParameter(L.url, "selarrow", selarrow)
                        });
                    })
                    ...
                    var c;		//$.jgrid.randId()，再有bug可以考虑这个算法
                    do {
                    	c = -Math.floor(new Date().getTime() + Math.random() * 100 + 100);
                    }
                    while ($.inArray(String(c), d) != -1);
                    ...
                    c = $.extend(true, {
                        ...
                        addParams: {position: "last",addRowParams: {extraparam: {}}},
                        editParams: {},
...
//                                    }
                                        ...
                                        if (p) {
                                            ...
                                            if ($.isFunction(c.delfunc)) {
                                                ...
                                                if (n.p.delurl == undefined || n.p.delurl == "clientArray") {
                                                    ...
                                                    if (p) {
                                                        var dD = {"id": p.join(",")}
                                                        $(n).ajaxPostURL(n.p.delurl, function (q) {
                                                            $.each(p, function (r, t) {
                                                                var t = $.trim(t);
                                                                var s = $(n).find("tr.jqgrow[id='" + t + "']");
                                                                if (q.userdata && q.userdata[t]) {
                                                                    var u = q.userdata[t];
                                                                    s.pulsate({color: "#bf1c56", repeat: 3})
                                                                } else {
            ...
            var x = $.extend(true, {}, aj.data("gridOptions"), E);
            var d = aj.attr("data-grid");
            var H = null;
            var ab = aj.attr("id") + "-context-menu-container";
            var X = null;
            var f = (d == "items" ? false : true);
            var I = (d == "items" ? false : true);
            var L = $.extend(true, {}, $.jgrid.defaults, {
                formatter: {
                ...
                rowList: d == "items" ? [] : [20, 50, 100, 200, 500, 1000, 2000],
                inlineNav: {
                    add: x.addurl || d == "items" ? true : false,
                    edit: x.editurl || d == "items" ? true : false,
...
//                                } else {
//                                    if (au.status == "0") {
//                                        Global.notify("error", au.msg)
//                                    } else {
//                                        Global.notify("error", "数据处理异常，请联系管理员",'系统错误')
//                                    }
//                                }
                    ...
                    editParams: {
                        restoreAfterError: false, beforeSaveRow: function (c) {
                            ...
                            if (L.editurl != "clientArray") {
                                ...
                                if (c.status == "1") {
                                    Global.notify('success', c.msg);
                                    //如果编辑完自动编辑下一行，注释下一行
                                    aw = false;
                                } else {
                                    if (c.status == "0") {
                                        Global.notify("error", c.msg);
                                        aw = false
                                    } else {
                                        Global.notify("error", "数据处理异常，请联系管理员",'系统错误');
                                        aw = false
                ...
                loadBeforeSend: function () {
                    //Metronic.blockUI(aj.closest(".ui-jqgrid"))
                    Metronic.blockUI({
                        target: aj.closest(".ui-jqgrid"),
                        boxed: true
                    });
                },
                ...
                beforeProcessing: function (au) {
                    if (au && au.content) {
                        ...
                        if (au.totalElements >= (2147473647 - 10000)) {
                            aj.jqGrid("setGridParam", {recordtext: "{0} - {1}\u3000"})
                        }
                    }
                },
                loadComplete: function (av) {
                    ...
                    if (av && av.content) {
                        $.each(av.content, function (aw, ax) {
                            aj.setRowData(ax.id, {_arrayIndex: aw})
                        });
                    ...
                    if (aa == "enable" && L.contextMenu && X.find("li").length > 0) {
                        aj.find("tr.jqgrow").each(function () {
                            $(this).contextmenu({
                                target: "#" + ab, onItem: function (ay, ax) {
                    ...
                    //Metronic.unblockUI();
                    Metronic.unblockUI(aj.closest(".ui-jqgrid"));
                    if($('.modal')){
                        $('.modal').modal('layout');
                    }
                },
                ...
                onSelectRow: function (av, c, au) {
                    ...
                    //aj.setGridHeight(200, true)
                },
                onCellSelect: function (au, c) {
                    aj.jqGrid("setGridParam", {iCol: c})
                },
            ...
            $.each(L.colModel, function (aw, c) {
                ...
                c = $.extend(true, {
                    ...
                    searchoptions: {
                        ...
                        buildSelect: function (aD) {
                            ...
                            for (var aA in aC) {
                                aA = aA + "";
                                aB += ("<option value='" + aC[aA] + "'>" + aA + "</option>")
                            }
                            aB += "</select>";
                            return aB
                        }
                ...
                if (c.formatter == "currency") {
                    c = $.extend({}, {width: 80, align: "right"}, c);
                    c.formatoptions = $.extend({}, c.formatoptions, {
                        decimalSeparator: ".",
                        thousandsSeparator: ",",
                        decimalPlaces: 2,
                        prefix: "",
                    ...
                    c.searchoptions = $.extend({}, c.searchoptions, {sopt: ["eq", "ne", "ge", "le", "gt", "lt"]})
                }
                if (c.formatter == "percentage") {
                    c = $.extend(true, {width: 50, align: "right"}, c);
                ...
                if (c.stype == "date" || c.sorttype == "date" || c.formatter == "date") {
                    c = $.extend(true, {width: 150, fixed: true, align: "center"}, c);
                    c.searchoptions = $.extend({}, c.searchoptions, {
                ...
                if (c.sorttype == "number" || c.edittype == "number" || c.formatter == "number") {
                    c = $.extend(true, {width: 60, align: "right", formatoptions: {defaultValue: ""}}, c);
                    c.searchoptions = $.extend({}, c.searchoptions, {sopt: ["eq", "ne", "ge", "le", "gt", "lt"]})
                }
                if (c.name == "id") {
                    c = $.extend(true, {
                        width: 80, align: "center", title: false, formatter: function (aD, aB, aF, aC) {
                            if (aD && aD.length > 5) {
                                var aA = aD.length;
                    ...
                    c.searchoptions = $.extend(true, c.searchoptions, {sopt: ["eq", "ne", "ge", "le", "gt", "lt"]})
                }
                ...
                if (c.edittype == "checkbox") {
                    c = $.extend(true, {formatter: "checkbox", stype: "select"}, c);
                    c.searchoptions.value = {"": "", "true": "Y", "false": "N"};
                    c.editoptions.value = "true:false"
                }
                if (c.edittype == undefined || c.edittype == "text" || c.edittype == "select" || c.edittype == "textarea") {
                    ...
                    c.editoptions = $.extend(c.editoptions, {
                        dataInit: function (aB) {
                            ...
                            aA.removeClass("editable").addClass("form-control").attr("autocomplete", "off").css({width: "100%"});
                            if (aA.is("input[type='text']")) {
                                if(c.editoptions.onlytext){
                                    var aBval = $.trim(aA.val());
                                    aA.val(aBval.replace(/<[^>]+>/g,""));
                                    aA.blur(function () {
                                        var aBval = $.trim(aA.val());
                                    });
                                } else {
                            ...
                            if (aA.is("select")) {
                                aA.select2({
                                    openOnEnter: false, placeholder: "请选择...", matcher: function (aE, aH) {
                                        var aD = makePy(aH) + "";
                ...
                if (c.formatter == "checkbox") {
                    c = $.extend(true, {width: 60, align: "center", editoptions: {value: "true:false"}}, c)
                }
                ...
                if (c.hasOwnProperty("searchoptions")) {
                    ...
                    if (au.hasOwnProperty("defaultValue") && au.defaultValue != "") {
                        ...
                        U[U.length++] = {field: ax, op: c.searchoptions.sopt[0], data: au.defaultValue}
                    }
                }
                L.colModel[aw] = c
            });
            if (!R) {
                L.colModel.push({label: "流水号", name: "id", hidden: true});
                if (L.colNames) {
                    L.colNames.push("流水号")
                }
            }
            if (d == "items") {
                L.colModel.push({name: "extraAttributes.dirtyRow", hidden: true, hidedlg: true});
                if (L.colNames) {
                    L.colNames.push("extraAttributes.dirtyRow")
                }
                L.colModel.push({name: "_arrayIndex", hidedlg: true, hidden: true});
                if (L.colNames) {
                    L.colNames.push("_arrayIndex")
                }
                L.colModel.push({name: "extraAttributes.operation", hidedlg: true, hidden: true});
                if (L.colNames) {
            ...
            if (L.jqPivot) {
                ...
                delete L.jqPivot;
                var m = L.url;
                L = {multiselect: false, pager: L.pager, shrinkToFit: false};
                aj.jqGrid("jqPivot", m, Q, L, {reader: "content"});
                return
            } else {
                aj.jqGrid(L)
            }
            ...
            if (L.setGroupHeaders) {
                aj.jqGrid("setGroupHeaders", $.extend(true, {useColSpanStyle: true}, L.setGroupHeaders))
            }
            aj.bindKeys({
                upKey: false, downKey: false, onEnter: function (au) {
                    if (au == undefined) {
                        return
                    }
                    aj.find("tr.jqgrow").attr("tabindex", -1);
            ...
            if (L.pager) {
                ...
                if (Util.notSmallViewport() && L.exportExcelLocal && d != "items") {
                    var e = {
                        ...
                        title: "导出当前列表数据",
                        onClickButton: function () {
                            if ($.isFunction(x.customeExportExcelLocal)) {
                                x.customeExportExcelLocal.call(x,aj);
                            } else {
                                aj.jqGrid("exportExcelLocal", L.exportExcelLocal,L)
                            }
                        }
                    };
                    if (L.pager) {
                ...
                if (L.pager) {
                    aj.jqGrid("navSeparatorAdd", L.pager, N);
                    aj.jqGrid("navButtonAdd", L.pager, N);
                    if (L.toppager) {
                        aj.jqGrid("navSeparatorAdd", L.toppager, N);
                    ...
                    var u = {
                        ...
                        onClickButton: function () {
                            ...
                            $.each(au, function (ax, aA) {
                                ...
                                var aw = $(aA);
                                if (az.length > 0) {
                                    var ay = $.extend({connectWith: az.join(",")}, am);
                ...
                aj.jqGrid("navSeparatorAdd", L.pager, {position: "first"});
                ...
                if (L.viewurl) {
                    ...
                    s.children("a").bind("click", function (ax) {
                        ...
                        if (az) {
                            ...
                            Global.addOrActiveTab(c, {title: "查看: " + av, url: aw})
                        }
                    });
                    G.push(s)
                }
                if (L.fullediturl) {
                    if (L.addable == undefined || L.addable != false) {
                        var ad = $('<li><a href="javascript:;" data-toggle="dynamic-tab" data-url="' + L.fullediturl + '"><i class="fa fa-plus-square"></i> 新增数据</a></li>').appendTo(X);
                        O.push(ad);
                        ...
                        at.children("a").bind("click", function (aw) {
                            ...
                            if (id = aj.getOnlyOneSelectedItem()) {
                                var au = L.cloneurl ? L.cloneurl : L.fullediturl;
                                var av = Util.AddOrReplaceUrlParameter(au, "id", id);
                                av = av + ("&clone=true");
                                var c = aj.closest(".tabbable").find(" > .nav");
                                Global.addOrActiveTab(c, {title: "克隆复制", url: av})
                            }
                        });
                        G.push(at)
                    }
                ...
                if (L.fullediturl) {
                    ...
                    r.children("a").bind("click", function (ax) {
                        ...
                        if (az) {
                            ...
                            var c = aj.closest(".tabbable").find(" > .nav");
                            Global.addOrActiveTab(c, {title: "编辑: " + av, url: aw})
                        }
                    });
                    G.push(r)
                }
                ...
                if (O.length > 0) {
                    $.each(O, function () {
                        ...
                        if (Util.notSmallViewport()) {
                            ...
                            color = 'blue';
                            console.log(au.attr('data-color'));
                            if(au.attr('data-color')){
                                 color = au.attr('data-color');
                            }
                            var c = "";
                            if (ax.attr("data-text") == "show") {
                                c = au.text()
                            }
                            var av = $('<button type="button" class="btn btn-xs '+color+'" style="margin-left:5px"><i class="' + aw + '"></i> ' + c + "</button>").appendTo(af.parent());
                            av.attr("title", ax.text());
                            if(ax.attr("open-type") != 'direct') {
                                av.click(function () {
                                    au.click()
                                });
                            }
                ...
                if (G.length > 0) {
                    ...
                    $.each(G, function () {
                        ...
                        if (Util.notSmallViewport() && av.attr("data-toolbar") == "show") {
                            ...
                            au.css({"margin-left": "5px"});
                            au.appendTo(af.parent());
                            if(av.attr("open-type") != 'direct') {
                ...
                if (z.length > 0) {
                    ...
                    $.each(z, function () {
                        var av = $(this);
                        color = 'blue';
                        if(av.attr('data-color')){
                            color = av.attr('data-color');
                        }
                        av.appendTo(X);
                        if (Util.notSmallViewport() && av.attr("data-toolbar") == "show") {
                            var c = av.children("a");
                            var au = c.clone();
                            au.addClass("btn btn-xs");
                            au.addClass(color);
                            au.css({"margin-left": "5px"});
                            au.appendTo(af.parent());
                            if(av.attr("open-type") != 'direct'){
                ...
                if (!Util.notSmallViewport()) {
                    ...
                    M.find("#" + M.attr("id") + "_left").css({width: "150px"})
                }
                if (L.pager && L.toppager) {
                    var t = aj.attr("id") + "_toppager";
                    var K = $("#" + t);
                    aj.jqGrid("navSeparatorAdd", "#" + t, {position: "first"});
            ...
            for (var ag = 0; ag < J.length; ag++) {
                ...
                if (ap.tooltips) {
                    ...
                    var $dom = 'jqgh' + '_' + $(W).attr('id') + '_' + v;
                    var ae = $('#' + $dom);
                    //var ae = $(".ui-jqgrid-sortable[id*='" + v + "']", y);
                    if (ae.size() > 0) {
                        ae.prepend(ah);
                        ah.tooltipster({contentAsHTML: true, offsetY: 5, theme: "tooltipster-punk"})
                    }
                }
            }
            ...
            if (ao == undefined && L.editurl && L.editurl != "clientArray") {
                //ao = L.editurl.substring(0, L.editurl.indexOf("!")) + "!buildValidateRules"
            }
            if (ao) {
                        ...
                        for (var av = 0; av < au.length; av++) {
                            ...
                            if ((aA.index && aA.index == aw) || (aA.name && aA.name == aw)) {
                                ...
                                if (aA.editrules.tooltips && aA.tooltips == undefined) {
                                    ...
                                    if (ay.size() > 0) {
                                        ay.prepend(az);
                                        az.tooltipster({contentAsHTML: true, offsetY: 5, theme: "tooltipster-punk"})
                                    }
            ...
            aj.jqGrid("gridResize", {minWidth: 500, minHeight: 100});
