var Page = function () {
    var initMultiimage = function () {
        $("[data-multiimage]").each(function () {
            ...
            var picsEditor = KindEditor.editor({allowFileManager: false});
            control.sortable({
                items: ".thumbnail", stop: function (event, ui) {
                    var index = 1000;
                    var $ui = $(ui.item);
    ...
    return {
        initAjaxBeforeShow: function ($container) {
            ...
            initMultiimage();
            // begin first table
            var $dataTable = $('#JS_table,.JS_table');
            if ($dataTable.length > 0) {
                $dataTable.each(function () {
                    var $el = $(this);
                    var dataPageLength = 50;
                    if ($el.attr("data-page-length")) {
                        dataPageLength = $el.attr("data-page-length");
                    }
                    // begin first table
                    $el.dataTable({
                        "aaSorting": [],
                        ...
                        "tableTools": {
                            "sSwfPath": themeUrl + "plugins/datatables/extensions/TableTools/swf/copy_csv_xls.swf",
                            "aButtons": [{
                                "sExtends": "csv",
                                "sButtonText": "CSV"
                            }, {
                    ...
                    $el.find('.group-checkable').change(function () {
                        var set = jQuery(this).attr("data-set");
                        var checked = jQuery(this).is(":checked");
                        jQuery(set).each(function () {
                            if (checked) {
                ...
                $("select", $container).each(function () {
                    ...
                    if ($select2.attr("data-select2-type") == "combobox") {
                        ...
                        options = $.extend({}, options, {
                            placeholder: "请选择或输入...", allowClear: true, query: function (query) {
                                var data = {results: []};
            ...
            //如果是select tag的功能，初始化
            $("input.select2tags", $container).each(function () {
                var $select2tags = $(this);
                $select2tags.select2({tags: $select2tags.attr("data-tags")})
            });
            //保存表单的数据为默认，暂时不用
            $("[data-profile-param]:not([data-profile-param-done])", $container).each(function () {
                ...
                $controls.add($el).mouseenter(function () {
                    $btn.toggle();
                    $btn.position({my: "right center", at: "left center", of: $controls});
                    $btn.off();
                    $btn.click(function () {
                        var codes = [];
                        var postData = {};
            ...
            $(".date-picker", $container).each(function () {
                ...
                if (BooleanUtil.toBoolean(timeInput)) {
                    $el.datetimepicker({autoclose: true, format: "yyyy-mm-dd hh:ii:ss", todayBtn: true, language: "zh-CN", minuteStep: 10})
                } else {
            ...
            //maxlength插件
            $("textarea[maxlength]", $container).maxlength({limitReachedClass: "label label-danger", alwaysShow: true});
            //dropdownselect插件
            ...
            $("[data-htmleditor='kindeditor']").each(function () {
                ...
                KindEditor.create($kindeditor, {
                    allowFileManager: false, width: "100%", height: height, minWidth: "200px", minHeight: "60px", afterBlur: function () {
                        this.sync()
                    }
                })
            });
            ...
            $("a.x-editable").each(function () {
                ...
                $edit.editable({
                    url: url, pk: pk, title: title, placement: placement, params: function (params) {
                        params.id = pk;
            ...
            $("[data-sigleimage]").each(function () {
                ...
                var options = {width: "150px", height: "150px"};
